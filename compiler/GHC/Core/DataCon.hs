...
module GHC.Core.DataCon (
        ...
        isUnboxedSumCon, isRefPrimCon, isRefUPrimCon,
...
-- are a little dodgy.
  ...
  where
    ...
    alt_arg_tys
      | is_mutable = pprTrace "alt_arg_tys: " (ppr (orig_arg_tys, -- TODO: RY debug tracing
    ...
    alt_rep_arg_tys -- Includes unpacking and strictness
      | is_mutable = pprTrace "alt_rep_arg_tys: " (ppr (orig_arg_tys, -- TODO: RY debug tracing
                        zipWith mkMutTys mutable_fields (dataConRepArgTys con)))
                   $ zipWith mkMutTys mutable_fields (dataConRepArgTys con)
        ...
        case orig_res_ty of
          TyConApp tc tys | tyConName tc /= tycon_name && is_mutable ->
               case tyConDataCons tc of
                 [dc] -> pprTrace "wrap_act = " (ppr (tc, tyConFamInst_maybe tc, -- TODO: RY Debug info
...
dataConInstArgTys dc@(MkData {dcUnivTyVars = univ_tvs,
 ...
 = ASSERT2( univ_tvs `equalLength` inst_tys
   ...
   map (substTyWith univ_tvs inst_tys) alt_arg_tys
...
dataConRepAltTys (MkData { dcRep = rep
  ...
  = case rep of  -- TODO: RY Get to the bottom of what is going on here :D.
      NoDataConRep -> pprTrace "dataConRepAltTys NoDataConRep" (ppr (orig_arg_tys, theta))
                        $ ASSERT( null eq_spec ) theta ++ orig_arg_tys
      DCR { dcr_arg_tys = arg_tys } -> pprTrace "dataConRepAltTys DCR" (ppr (arg_tys, orig_arg_tys, theta))
                                        $ theta ++ orig_arg_tys  -- arg_tys
...
isUnboxedTupleCon (MkData {dcRepTyCon = tc})
